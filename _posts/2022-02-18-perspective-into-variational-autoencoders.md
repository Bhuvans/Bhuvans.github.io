...
quantity that is then maximized wrt $\theta$ to give the maximum likelihood parameter estimate. $$ \widehat{\theta_{ML}}=argmax_\theta\sum_{x\in X} log P\left(x\middle|\theta\right) $$
